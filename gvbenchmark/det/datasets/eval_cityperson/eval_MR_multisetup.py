...
class COCOeval:
    ...
    def evaluate(self, id_setup):
        ...
        results (a list of dict) in self.evalImgs
        :return: None
        '''
        ...
        # print('DONE (t={:0.5f}s).'.format(toc-tic))
    ...
    def evaluateImg(self, imgId, catId, hRng, vRng, maxDet):
        ...
        dtind = [
            i for i in range(len(dt))
            if dt[i]['height'] >= hRng[0] / self.params.expFilter
            and dt[i]['height'] < hRng[1] * self.params.expFilter
        ]
        dt = [
    ...
    def summarize(self, id_setup, res_file):
        ...
        def _summarize(iouThr=None, maxDets=100):
            p = self.params
            iStr = ' {:<18} {} @ {:<18} [ IoU={:<9} \
                 | height={:>6s} | visibility={:>6s} ] = {:0.5f}%'

            titleStr = 'Average Miss Rate'
            typeStr = '(MR)'
            setupStr = p.SetupLbl[id_setup]
            iouStr = '{:0.5f}:{:0.5f}'.format(p.iouThrs[0], p.iouThrs[-1]) \
                if iouThr is None else '{:0.5f}'.format(iouThr)
            heightStr = '[{:0.0f}:{:0.0f}]'.format(p.HtRng[id_setup][0],
                                                   p.HtRng[id_setup][1])
            occlStr = '[{:0.5f}:{:0.5f}]'.format(p.VisRng[id_setup][0],
                                                 p.VisRng[id_setup][1])
    ...
    def summarize_nofile(self, id_setup):
        """Compute and display summary metrics for evaluation results.
